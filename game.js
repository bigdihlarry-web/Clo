...
(() => {
  ...
  // Pre-roll single die for both players to decide who goes first
  function doPreRoll() {
  ...
  function playerRollMain(playerId) {
    ...
    if (state.cheatFlags[playerId].main && playerId === 2 && state.player[1].lastEval) {
    ...
    if (playerId === 1 && state.cheatFlags[1].main) {
    ...
    state.player[playerId].cheatedThisRound = state.player[playerId].cheatedThisRound || (state.cheatFlags[playerId].main === false && options.favor === 'player1' ? true : false);
    // Important: if player clicked cheat (preferCheat), we set cheatedThisRound true before consuming; we already consumed above.
    // But to keep tracking consistent, set to true if they had cheat flag used (we stored none). To be safe, set to true if their lastEval matches a "cheat-like" strong result and the cheat flag was likely used earlier—this is not critical.
  ...
  // Cheat button: toggle cheat flag for current player/phase
  cheatBtn.addEventListener('click', () => {
    if (state.phase === 'pre-roll') {
      // Activate cheat for both players in pre-roll (or just current if you prefer)
      // For simplicity, let's assume player 1 gets cheat in pre-roll when button is clicked
      state.cheatFlags[1].pre = true;
      setMessage('Cheat activated for Player 1 pre-roll! (Will roll 5 or 6)');
    } else if (state.phase === 'main' && state.currentTurn) {
      // Activate cheat for current turn player
      state.cheatFlags[state.currentTurn].main = true;
      state.player[state.currentTurn].cheatedThisRound = true;
      setMessage(`Cheat activated for Player ${state.currentTurn}! (Will get favorable roll)`);
    }
  });
